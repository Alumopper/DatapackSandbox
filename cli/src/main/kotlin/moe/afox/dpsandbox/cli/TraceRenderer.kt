...
object TraceRenderer {
    fun render(event: CommandTraceEvent): String {
        ...
        return "${ConsoleStyle.dim(
            "[${event.tick}]",
        )} trace $status ${ConsoleStyle.bold(
            event.command,
        )} commands=${event.commandsExecuted} outputs=${event.outputs}$changes$source$stack$error"
    }
