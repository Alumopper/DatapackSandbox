...
import net.datapacksandbox.core.JsonValues
import net.datapacksandbox.core.PlayerEvents
import net.datapacksandbox.core.ResourceLocation
import net.datapacksandbox.core.SandboxException
import net.datapacksandbox.core.UnsupportedFeatureMode
import net.datapacksandbox.core.VersionProfiles
import net.datapacksandbox.core.createSandbox
import net.datapacksandbox.core.toJson
import net.datapacksandbox.core.toPlayerJson
...
class AdvancementCommand : CliktCommand(name = "advancement") {
    ...
    override fun run() {
        try {
            val sandbox = createSandbox(version, packs, unsupportedFeatureMode = unsupportedFeatureMode(unsupported))
            val action = args.getOrNull(0) ?: throw SandboxException(net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT, "advancement action is required")
            ...
            when (action) {
                ...
                "progress" -> {
                    val progress = sandbox.advancements.progress(player, id ?: throw SandboxException(net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT, "advancement id is required"))
                    println(JsonValues.render(player.toPlayerJson()))
                    println(progress.criteria)
                }
                "grant" -> sandbox.advancements.grant(player, id ?: throw SandboxException(net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT, "advancement id is required"), args.getOrNull(3)).forEach { println(it) }
                "revoke" -> sandbox.advancements.revoke(player, id ?: throw SandboxException(net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT, "advancement id is required"), args.getOrNull(3)).forEach { println(it) }
                else -> throw SandboxException(net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT, "Unknown advancement action '$action'")
            }
        } catch (error: SandboxException) {
            println(ConsoleStyle.diagnostic(error.render()))
            exitProcess(ExitCodes.forException(error))
    ...
    override fun run() {
        try {
            ...
            if (args.getOrNull(0) != "player") {
                throw SandboxException(net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT, "Usage: event player <name> <type> [id] [action]")
            }
            val player = sandbox.createPlayer(args.getOrNull(1) ?: "Steve")
...
fun unsupportedFeatureMode(value: String): UnsupportedFeatureMode =
    when (value.lowercase()) {
        ...
        "error", "strict" -> UnsupportedFeatureMode.ERROR
            ...
            net.datapacksandbox.core.DiagnosticCode.INPUT_FORMAT,
