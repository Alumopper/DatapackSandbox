...
- 完善玩家模型：
  ...
  - 玩家 NBT 默认只读；可通过命令、fixture 或事件改变玩家状态；新建玩家会使用当前 `defaultGameMode`；world fixture、manifest world 和 quick-test world builder 可声明玩家末影箱物品，并通过 snapshot、完整 NBT path、`assertPlayer`/manifest player assertion 和 `assertItem`/manifest item assertion 检查。
- 完善 item stack：
