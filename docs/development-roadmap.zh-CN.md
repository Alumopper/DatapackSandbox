...
- 扩展 `recipe`、`advancement`、`attribute`、`damage`、`effect`、`enchant`、`random`、`team`、`bossbar`、`worldborder` 的子命令覆盖；`recipe give/take` 已维护玩家 recipe 集合，并在结构化输出中报告 changed 数量和实际变更的 recipe id 列表，便于生成器断言 `*` 展开结果。
  ...
  - `place structure` 和 `place template` 已可把加载到 `worldgen/structure` 的沙盒结构 JSON（`blocks`/`entities`）按命令坐标展开到 sparse world，并记录 `placed`、`changedBlocks`、`skippedBlocks`、`entities`、变化坐标和实体目标；`place template` 支持确定性 rotation、mirror、integrity 和 seed 参数；缺失或非沙盒结构资源，以及 `place feature|jigsaw` 仍保留结构化 worldgen intent 输出，便于命令生成器验证放置目标、位置和额外参数。
...
- worldgen/structure 更深入模拟；`place structure` / `place template` 已覆盖沙盒结构 JSON 的方块/实体落地、基础旋转/镜像和 integrity 过滤，后续可继续扩展原版结构 NBT、processor、feature/jigsaw 语义。
- 可选外部差分验证。
- 高级性能缓存。
- Maven 发布和更完整的平台测试。
