...
- 增加命令 trace：
  ...
  - CLI `run`/`check` 的 `--trace-filter` 已支持 `selector=`/`target=`、文本型 `output=`、数量/布尔型 `outputs=`，并在 trace JSON 中保留每条命令产生的 `outputEvents`。
...
- 增强 `run`：
  ...
  - `--assert`/`--assert-file` 已支持 score、storage、advancement、player、item、entity、diff、event-trace、trace、trace-output、warning、unsupported、output、output-normalized 和 output-payload 简写，适合命令生成器结果的快速回归。
- 增强 REPL：
  - `inspect` 输出结构更稳定。
