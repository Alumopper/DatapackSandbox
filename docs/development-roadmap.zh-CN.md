...
- 扩展 `assertions`：
  ...
  - 支持 equals、contains、exists、missing、count、min/max、matches、path；output assertion 已支持 plain/normalized text 的正则 `matches`，segment assertion 也可按原始或 normalized segment text 做正则匹配。
- 支持 manifest include：
