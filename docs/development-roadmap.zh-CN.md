...
- 扩展 `execute`：
  ...
  - 常用 selector 选项 `name`、`gamemode`、`team`、`scores`、`level`、`x_rotation/y_rotation`、`sort`、`distance`、`x/y/z`、`dx/dy/dz` 已接入确定性过滤和排序；`scores={...}` 支持花括号内多 objective 整数范围，等级和旋转范围可用于玩家/实体状态筛选，`sort=random` 使用稳定顺序，适合命令生成器回归和范围选择调试。
- 扩展 `data`：
