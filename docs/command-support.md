...
| `execute` | Partial | `as`, `at`, `positioned`, `align`, `anchored`, `facing`, `in`, `rotated`, `store`, `if`, `unless`, `run`; conditions support `entity`, `score`, `data`, `block`, `predicate`, `dimension`, `biome`, and `loaded`. |
