...
| `data` | Partial | `get` with optional numeric scale, `merge`, `modify`, `remove` for `storage`, `entity`, and `block`; `merge` records structured before/after output; paths support fields, positive/negative numeric indexes, and simple object matchers; `modify` supports `value`, `from`, and `string` sources; top-level NBT is schema-checked. |
