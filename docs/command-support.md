...
| `scoreboard` | Partial | `modeled` | Objectives `add`, `remove`, `list`, `setdisplay`; display slots are stored in snapshots and mutations record structured output; players `set`, `add`, `remove`, `get`, `reset`, `list`, `enable`, `operation`; `players get` records a structured data output for assertions and `execute store result`. |
...
| `inspect <...>` | `modeled` | Inspect score, storage, random sequences, scheduled functions, entities, blocks, player, loot, predicate, advancement, recipe, item_modifier, raw JSON resources, tags, resource index, registry groups, outputs, and player event traces. |
