...
| `place` | Partial | `modeled` | `place structure <id> [pos]` and `place template <id> [pos] [rotation] [mirror] [integrity] [seed]` apply loaded sandbox structure JSON resources (`worldgen/structure` with `blocks`/`entities`) to the sparse world and record changed blocks/entities; structure JSON can reference `worldgen/processor_list` resources for `block_ignore`, `protected_blocks`, `jigsaw_replacement`, and simple rule replacement processors. Template placement supports deterministic `none`/90-degree rotations, `front_back`/`left_right` mirroring, and integrity filtering. `place jigsaw <pool> <target> <maxDepth> [pos]` resolves loaded `worldgen/template_pool` single/legacy structure elements, applies element processors, and places the selected structure. `place feature <id> [pos]` resolves loaded `worldgen/placed_feature`/`worldgen/configured_feature` simple_block, block_column, deterministic disk/vegetation_patch/tree/replacement/selector/random_patch/flower JSON, and sparse-world ore targets, placing or replacing one or more blocks around the command position. Missing or unsupported resources still record structured worldgen intent with `placed=false`. |
