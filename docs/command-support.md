...
| `inspect <...>` | `modeled` | Inspect score, storage, gamerules, random sequences, scheduled functions, forced chunks, scoreboard objectives/displays, entities, blocks, player, loot, predicate, advancement, recipe, item_modifier, raw JSON resources, tags, resource index, registry groups, outputs, and player event traces. |
