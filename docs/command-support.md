...
| `item` | Partial | `replace entity|block ... with <item> [count]`; `modify entity|block ... <modifier>` applies common item modifier functions (`set_components`, `set_custom_data`, `set_count`, `set_damage`, `set_name`, `set_lore`). |
