...
| `datapack` | 部分支持 | `list` 报告已加载 typed/raw/tag/resource-index 资源数量；`enable`/`disable` 作为 no-op 接受，因为沙盒创建后 pack 顺序固定。 |
...
| CLI `check <manifest-or-directory>` | 运行 `.dps.json` 清单；可用 `--snapshot-diff-on-fail` 输出状态差异，也可用 `--trace-file` 和 `--trace-filter` 写出过滤后的 JSONL 命令 trace。 |
