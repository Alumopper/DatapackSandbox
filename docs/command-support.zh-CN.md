...
| `data` | 部分支持 | `get`、`merge`、`modify`、`remove`，目标支持 `storage`、`entity`、`block`；`modify` 支持 `value`、`from` 和 `string` 来源；顶层 NBT 字段经过 schema 校验。 |
...
| `item` | 部分支持 | `replace entity <targets> <slot> with <item> [count]`；`modify entity <targets> <slot> <modifier>` 会应用常用 item modifier 函数（`set_components`、`set_custom_data`、`set_count`）。 |
| `kick` | 未支持 | 不模拟网络会话。 |
| `kill` | 支持 | 移除选中的沙盒实体。 |
