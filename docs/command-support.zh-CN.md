...
| `scoreboard` | 部分支持 | `modeled` | objectives 支持 `add`、`remove`、`list`、`setdisplay`；display slot 会进入 snapshot，修改会记录结构化输出；players 支持 `set`、`add`、`remove`、`get`、`reset`、`list`、`enable`、`operation`；`players get` 会记录结构化 data 输出，供断言和 `execute store result` 使用。 |
...
| `inspect <...>` | `modeled` | 查看 score、storage、random sequence、scheduled function、entities、blocks、player、loot、predicate、advancement、recipe、item_modifier、raw JSON resource、tags、resource index、registry group、outputs 和玩家事件 trace。 |
