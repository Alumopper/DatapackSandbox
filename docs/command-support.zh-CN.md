...
| `data` | 部分支持 | 支持带可选数值 scale 的 `get`，以及 `merge`、`modify`、`remove`，目标支持 `storage`、`entity`、`block`；path 支持字段、数字索引和简单对象匹配；`modify` 支持 `value`、`from` 和 `string` 来源；顶层 NBT 字段经过 schema 校验。 |
...
| `item` | 部分支持 | `replace entity|block ... with <item> [count]` 和 `from entity|block ...`；`modify entity|block ... <modifier>` 会应用常用 item modifier 函数（`set_components`、`set_custom_data`、`set_count`、`limit_count`、`set_damage`、`set_name`、`set_lore`、`filtered`、`reference`、`sequence`）。 |
| `kick` | 未支持 | 不模拟网络会话。 |
...
| `loot` | 部分支持 | 支持 `give`、`insert`、`spawn`、`replace entity`、`replace block`；source 支持 `loot <table>`、`fish <table> <pos> [tool]`、`mine <pos> [tool]`，以及实体声明 `DeathLootTable` 时的 `kill <target>`；常用函数覆盖 count、components/custom data、damage、name 和 lore。 |
| `me` | 支持 | 记录为 chat 输出事件。 |
