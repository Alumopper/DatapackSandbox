...
| `item` | 部分支持 | `replace entity|block ... with <item> [count]`；`modify entity|block ... <modifier>` 会应用常用 item modifier 函数（`set_components`、`set_custom_data`、`set_count`、`set_name`、`set_lore`）。 |
| `kick` | 未支持 | 不模拟网络会话。 |
