...
| Check a full datapack with resources, commands, and assertions | `.dps.json` manifest | [Manifest Regression Tests](/en/workflows/manifest-tests) |
| Run one function or command and inspect output or snapshots | CLI / REPL | [Run Datapacks with the CLI](/en/workflows/cli) |
...
To try the CLI immediately, prepare the fat JAR as described in [Install and Obtain](/en/workflows/installation), then open the interactive REPL:
...
- For predefined worlds, read the [QuickTest Fixture Reference](/en/reference/quicktest-fixtures).
- For assertions, read the [QuickTest Assertion Reference](/en/reference/quicktest-assertions).
