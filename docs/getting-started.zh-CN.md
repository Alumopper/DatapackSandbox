...
| 检查一个完整数据包的资源、命令和断言 | `.dps.json` manifest | [Manifest 回归测试](/workflows/manifest-tests) |
| 临时跑一个函数或命令，观察输出和 snapshot | CLI / REPL | [使用 CLI 运行数据包](/workflows/cli) |
...
如果你只是想立即体验 CLI，先按 [安装与获取](/workflows/installation) 准备 fat JAR，然后打开交互式 REPL：
...
JVM 测试应依赖 `testkit` artifact；它会传递引入 `core`，不应依赖 CLI fat JAR。
...
- 需要准备世界状态：读 [QuickTest fixture 参考](/reference/quicktest-fixtures)。
- 需要查断言：读 [QuickTest 断言参考](/reference/quicktest-assertions)。
