...
class LootEngine(
    ...
    private fun generateTable(
        table: LootTable,
        context: LootContext,
        stack: MutableSet<ResourceLocation>,
    ): LootResult {
                    ...
                    if (entry != null) {
                        val poolFunctions =
                            pool
                                .arrayObjects(
                                    "functions",
                                ).map { LootFunction(canonical(it.requiredString("function")), it) }
    ...
    private fun expandEntry(
        ...
        val base =
            when (type) {
                ...
                "loot_table" ->
                    generateTable(
                        datapack.lootTable(ResourceLocation.parse(entry.requiredString("value"))),
                        context,
                        stack,
                    ).items
                ...
                "tag" -> {
                    ...
                    "Loot entry type '${entry.requiredString("type")}' is not implemented",
                )
            }
        return applyFunctions(base, functions, context, random)
    }

    private fun expandItemTag(
        id: ResourceLocation,
        stack: MutableSet<ResourceLocation>,
                    ...
                    if (nestedTag == null) {
                        if (value.required) {
                            throw SandboxException(
                                ...
                                "Item tag '$nested' referenced by '$id' was not found",
                            )
                        }
                        emptyList()
                    } else {
    ...
    private fun findItemTag(id: ResourceLocation): TagDefinition? = datapack.tags[TagKey("item", id)] ?: datapack.tags[TagKey("items", id)]

    private fun testConditions(
        conditions: JsonElement?,
        context: LootContext,
    ): Boolean = predicates.testConditions(conditions, context.predicateContext)
    ...
    ): List<ItemStack> = items.flatMap { item -> applyFunctions(item, functions, context, random, activeReferences) }
    ...
    private fun applyFunctions(
        ...
        functions.forEach { function ->
            ...
            when (function.type) {
                ...
                "set_components" ->
                    function.root.getAsJsonObject("components")?.entrySet()?.forEach { (key, value) ->
                        stack.components.add(key, value.deepCopy())
                    }
                "set_custom_data", "set_nbt" ->
                    function.root.string("tag")?.let {
                ...
                "filtered" -> {
                    ...
                    val inner =
                        function.root.getAsJsonObject("function")
                            ?: throw SandboxException(DiagnosticCode.INPUT_FORMAT, "filtered loot function requires nested function")
                    stack =
                        applyFunctions(
                            stack,
                            listOf(LootFunction(canonical(inner.requiredString("function")), inner)),
                            context,
                            random,
                            activeReferences,
                        ).firstOrNull()
                ...
                element.asJsonArray.mapIndexed { index, item ->
                    if (!item.isJsonObject) {
                        throw SandboxException(
                            ...
                            "Referenced loot function entry at index $index must be an object",
                        )
                    }
    ...
    private fun applyBonus(
        ...
        return when (formula) {
            ...
            "uniform_bonus_count" -> {
                val multiplier =
                    parameters.get("bonusMultiplier")?.asInt
    ...
    private fun copyNameFromContext(
        stack: ItemStack,
        function: LootFunction,
        context: LootContext,
    ) {
    ...
    private fun entityNameSource(
        ...
        when (source) {
            ...
            "attacking_player", "killer_player" ->
                context.predicateContext.attackingPlayer
    ...
    private fun componentSource(
        ...
        val playerItem =
            when (source) {
                "this" ->
                    (context.predicateContext.thisEntity as? SandboxPlayer)?.selectedItem
                        ?: context.predicateContext.player?.selectedItem
            ...
            element.isJsonArray ->
                element.asJsonArray.mapIndexed { index, value ->
                    if (!value.isJsonPrimitive || !value.asJsonPrimitive.isString) {
                        throw SandboxException(
                            DiagnosticCode.INPUT_FORMAT,
                            "copy_components '$key' entries must be strings at index $index",
                        )
                    }
    ...
    private fun rollCount(
        element: JsonElement?,
        random: Random,
    ): Int = floor(rollDouble(element, random)).toInt()

    private fun rollDouble(
        element: JsonElement?,
        random: Random,
    ): Double =
        when {
    ...
    private fun tableError(
        table: LootTable,
        message: String,
    ): SandboxException = SandboxException(DiagnosticCode.INPUT_FORMAT, message, SourceLocation(file = table.file))
