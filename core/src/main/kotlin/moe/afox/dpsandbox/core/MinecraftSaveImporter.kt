...
object MinecraftSaveImporter {
     ...
     */
    fun importInto(world: SandboxWorld, profile: VersionProfile, options: MinecraftSaveImportOptions): MinecraftSaveImportResult {
    ...
    private fun importBlocks(chunkNbt: JsonObject, requestedChunk: ChunkPos, world: SandboxWorld): Int {
        ...
        sections.forEach { sectionElement ->
            ...
            for (index in 0 until 4096) {
                val paletteIndex = if (palette.size() == 1) {
                    0
                } else {
                    packedIndex(packed ?: throw SandboxException(DiagnosticCode.INPUT_FORMAT, "Chunk section has palette size ${palette.size()} but no block state data"), index, bits)
                }
    ...
    private fun importBlockEntities(chunkNbt: JsonObject, world: SandboxWorld, profile: VersionProfile): Int {
        val blockEntities = chunkNbt.array("block_entities")
        ...
        blockEntities.forEach { element ->
            ...
            val block = world.block(pos)
                ?: throw SandboxException(DiagnosticCode.INPUT_FORMAT, "Block entity at $pos has no matching block; import blocks with block entities")
    ...
    private fun importEntities(chunkNbt: JsonObject, world: SandboxWorld, profile: VersionProfile, dimension: ResourceLocation): Int {
        ...
        entities.forEach { element ->
            ...
            val uuid = nbt.get("UUID")?.let { JsonValues.render(it) } ?: java.util.UUID.randomUUID().toString()
    ...
    private fun regionFile(regionDir: Path, chunk: ChunkPos, cache: MutableMap<Path, AnvilRegionFile?>): AnvilRegionFile? {
    ...
    private fun dimensionRoot(saveRoot: Path, dimension: ResourceLocation): Path =
        when (dimension.toString()) {
            ...
            else -> saveRoot.resolve("dimensions").resolve(dimension.namespace).resolve(dimension.path.replace('/', java.io.File.separatorChar))
        }
    ...
    private fun bitsPerBlock(paletteSize: Int): Int =
        maxOf(4, ceil(ln(paletteSize.toDouble()) / ln(2.0)).toInt())

    private fun packedIndex(data: JsonArray, index: Int, bits: Int): Int {
