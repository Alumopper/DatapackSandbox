...
export class SandboxPanel implements vscode.Disposable {
  ...
  private async perform(action: string, payload: Record<string, unknown>): Promise<unknown> {
    switch (action) {
      case "status": return this.client.hasActiveSandbox ? { active: true, data: await this.inspect() } : { active: false };
      case "start": return { active: true, data: await this.start(payload) };
      case "stop": this.client.close(); return { active: false };
      case "reload": await this.client.request("reload", { keepWorld: payload.keepWorld !== false }); return { active: true, data: await this.inspect() };
      case "command": await this.client.request("runCommand", { command: String(payload.value ?? "") }); return { active: true, data: await this.inspect() };
      case "function": await this.client.request("runFunction", { id: String(payload.value ?? "") }); return { active: true, data: await this.inspect() };
      case "tick": await this.client.request("tick", { count: Number(payload.value || 1) }); return { active: true, data: await this.inspect() };
      case "load": await this.client.request("load"); return { active: true, data: await this.inspect() };
      case "event": await this.client.request("injectPlayerEvent", { event: parseJsonPayload(payload.value, "Player event") }); return { active: true, data: await this.inspect() };
      case "fixture": await this.client.request("applyWorldFixture", { path: String(payload.value ?? "") }); return { active: true, data: await this.inspect() };
      case "reset": await this.client.request("resetWorld"); return { active: true, data: await this.inspect() };
      case "inspect": return { active: true, data: await this.inspect() };
...
<body>
  <main class="shell">
    ...
    <div class="grid">
      <section class="card">
        ...
        <div class="card-body">
          <div class="field"><label for="version">Minecraft profile</label><input id="version" value="${version}" autocomplete="off"></div>
  ...
  <script nonce="${nonce}">
    ...
    const byId=(id)=>document.getElementById(id);const tabs=byId('tabs'),content=byId('content'),toast=byId('toast'),valueInput=byId('value'),suggestions=byId('suggestions'),feedback=byId('commandFeedback');
    ...
    function send(action,extra={}){const requestId=++sequence;pending++;setBusy(true);api.postMessage({action,requestId,payload:{...payload(),...extra}})}
    ...
    window.addEventListener('message',(event)=>{const message=event.data;if(message.action==='completions'||message.action==='checkCommand'){if(message.requestId!==latestAssist[message.action])return;if(!message.ok){if(message.action==='checkCommand')showCheck({valid:false,message:message.error||'Check failed'});return}if(message.action==='completions')showSuggestions(message.result);else showCheck(message.result);return}pending=Math.max(0,pending-1);if(!message.ok){setBusy(false);notify(message.error||'Operation failed',true);return}if(message.action==='openSource'){setBusy(false);return}const result=message.result||{};if(typeof result.active==='boolean')setActive(result.active);if(result.data)data=result.data;if(message.action!=='status')notify(actionLabel(message.action));render();if(message.action==='start'||message.action==='command'||message.action==='reset'||message.action==='reload')scheduleAssist()});
