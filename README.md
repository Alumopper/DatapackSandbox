...
List resource support and behavior levels:
...
- configurable unsupported-command policy: `warn` by default, `error` for strict
  validation, `ignore` for silent skipping; `run --strict` and `check --strict`
  also fail missing resource references during generator-output checks
